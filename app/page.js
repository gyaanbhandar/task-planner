    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000000aa' }}
      ...
      <div style={{ background: '#1A1D27', borderRadius: 16, padding: 24, width: isMobile ? '92%' : 440, maxHeight: '90vh', overflowY: 'auto', border: '1px solid #2a2d3a' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#8B8D97', fontSize: 22, cursor: 'pointer' }}>✕</button>
        </div>
...
export default function Home() {
  ...
  const updateTask = async () => {
    ...
    const { error } = await supabase.from('tasks').update({
      title: form.title, description: form.description, category: form.category,
      subcategory: form.subcategory, priority: form.priority, type: form.type, deadline: form.deadline || null,
    }).eq('id', editTask.id);
    if (!error) {
      await loadTasks();
      setEditTask(null);
  ...
  const getAiPlan = async () => {
    ...
    try {
      const res = await fetch('/api/ai-plan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tasks: summary }) });
      const data = await res.json();
      setAiPlan(data.plan || 'Response nahi aaya.');
    } catch (e) { setAiPlan('AI connect nahi hua. Khud prioritize karo.'); }
    setAiLoading(false);
  };
        ...
        <div style={{ width: 240, background: '#14161F', borderRight: '1px solid #1e2130', position: 'sticky', top: 0, height: '100vh', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: '4px 8px', flex: 1 }}>
            {[{ id: 'today', icon: '📋', label: "Today's Focus" }, { id: 'all', icon: '📁', label: 'All Tasks' }, { id: 'ai', icon: '🤖', label: 'AI Plan' }].map(item => (
              <button key={item.id} onClick={() => { setView(item.id); setSelectedCat(null); }} style={{
            ...
            {CATEGORIES.map(c => (
              <button key={c.id} onClick={() => { setView('category'); setSelectedCat(c.id); setFilter('all'); setSubFilter(''); }} style={{
      ...
      {isMobile && sidebarOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex' }}>
          <div style={{ width: 260, background: '#14161F', height: '100%', overflowY: 'auto', borderRight: '1px solid #1e2130', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '4px 8px', flex: 1 }}>
              {[{ id: 'today', icon: '📋', label: "Today's Focus" }, { id: 'all', icon: '📁', label: 'All Tasks' }, { id: 'ai', icon: '🤖', label: 'AI Plan' }].map(item => (
                <button key={item.id} onClick={() => { setView(item.id); setSelectedCat(null); setSidebarOpen(false); }} style={{
                  display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, marginBottom: 2,
                  background: view === item.id && !selectedCat ? '#6C5CE718' : 'transparent', color: view === item.id && !selectedCat ? '#a29bfe' : '#8B8D97',
                }}><span style={{ fontSize: 16 }}>{item.icon}</span>{item.label}</button>
              ))}
              ...
              <div style={{ padding: '4px 12px', fontSize: 10, color: '#555', fontWeight: 700, letterSpacing: 1 }}>CATEGORIES</div>
              {CATEGORIES.map(c => (
                <button key={c.id} onClick={() => { setView('category'); setSelectedCat(c.id); setFilter('all'); setSubFilter(''); setSidebarOpen(false); }} style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '9px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 13, marginBottom: 1,
                  background: selectedCat === c.id ? c.color + '14' : 'transparent', color: selectedCat === c.id ? c.color : '#8B8D97',
                }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 15 }}>{c.icon}</span>{c.name}</span>
