  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: VISUAL_THEME.sidebar, borderRight: `1px solid ${VISUAL_THEME.border}`, padding: '24px 16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px', paddingLeft: '8px' }}>
        ...
        <div>
          <h2 style={{ fontSize: '15px', fontWeight: 700, color: VISUAL_THEME.text, margin: 0 }}>Business OS</h2>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', flex: 1, overflowY: 'auto' }}>
        {[
          ...
          <button key={v.id} onClick={() => onViewChange(v.id, null, null)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '8px', border: 'none', background: currentView === v.id && !activeCategory && !activeClient ? 'rgba(99, 102, 241, 0.08)' : 'transparent', color: currentView === v.id && !activeCategory && !activeClient ? VISUAL_THEME.accent : VISUAL_THEME.textSec, fontSize: '13px', fontWeight: 600, cursor: 'pointer', textAlign: 'left' }}>
        ...
        <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: `1px solid ${VISUAL_THEME.border}` }}>
          <button onClick={() => onViewChange('manage_categories', null, null)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px 12px', width: '100%', border: 'none', background: 'transparent', fontSize: '13px', color: VISUAL_THEME.textSec, cursor: 'pointer', textAlign: 'left' }}>📂 Manage Categories</button>
          <button onClick={() => onViewChange('notifications', null, null)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '8px 12px', width: '100%', border: 'none', background: 'transparent', fontSize: '13px', color: VISUAL_THEME.textSec, cursor: 'pointer', textAlign: 'left' }}>🔔 Notifications Alerts</button>
        </div>
      </div>
      ...
      <div style={{ marginTop: '16px', borderTop: `1px solid ${VISUAL_THEME.border}`, paddingTop: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
          <div style={{ width: '32px', height: '32px', background: 'rgba(99,102,241,0.1)', color: VISUAL_THEME.accent, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '12px', flexShrink: 0 }}>A</div>
          <span style={{ fontSize: '12px', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{userName ? userName.split('@')[0] : 'Anukant'}</span>
        </div>
        <button onClick={onLogout} style={{ background: '#FEE2E2', border: 'none', borderRadius: '6px', width: '28px', height: '28px', color: '#EF4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>⏻</button>
      </div>
    </div>
  );
}
